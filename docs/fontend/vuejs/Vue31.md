...
可以再次使用 nvm -v 命令进行验证，成功显示nvm版本号。
...
- `Vue2` 的配置（`data`、`methos`......）中**可以访问到** `setup`中的属性、方法（可以使用this）。
- 但在`setup`中**不能访问到**`Vue2`的配置（`data`、`methos`......）（setup中不能使用this）。
...
- **返回值：**一个`Proxy`的实例对象(原生JS中也有Proxy)，简称：响应式对象。

![image-20240408194449460](images/image-20240408194449460.png)
...
- `reactive` 只能定义对象类型数据
- 若`ref`接收的是对象类型，内部 `.value` 其实也是调用了`reactive`函数。

![image-20240408202817033](images/image-20240408202817033.png)
...
>
>    ```
>    // Object.assign 方法会将newObj对象加到oldObj上
...
>
>    ```
>    a.value = newObj. // 这样是因为value 是响应式的
>    ```
>
>    
...
> 3. 若需要一个响应式对象，且层级较深，推荐使用`reactive`（表单推荐）。

## 3.7. 【toRefs 与 toRef】

- 作用：将一个响应式对象中的每一个属性，转换为`ref`对象。
- 从响应式的对象中解构出来一个对象，该对象会失去响应式式，必须用 `toRefs` 或`toRef`
...
> 备注3: 传参时不能传数组
>
> ![image-20240414170924732](images/image-20240414170924732.png)
